 ...
 *   mgr.destroy();
 *
 * React Native port of src/lib/tradeStreamManager.ts
 * ────────────────────────────────────────────────────
 * Changes vs the web original:
 *   1. Import path: `./realtimeTradeAggregator.js` → `./realtimeTradeAggregator`
 *      (Metro bundler resolves .ts extensions without the explicit .js suffix).
 *
 * All business logic, latency tracking, deduplication, and pub-sub APIs are
 * preserved exactly.
 */

import { toMs } from "./realtimeTradeAggregator";
