 ...
 *
 * React Native port of src/lib/marketSession.ts
 * ─────────────────────────────────────────────
 * No modifications — this file uses only React hooks (useEffect, useState)
 * and standard JavaScript Date / setInterval / clearInterval APIs, all of
 * which are fully supported by Hermes (React Native's JS engine).
 * There are no DOM APIs, browser globals, or HTML-specific constructs.
 *
 * Note: useRef is present in the web source's import but unused in the
 * function bodies; it is omitted here to keep the import list clean.
 */
