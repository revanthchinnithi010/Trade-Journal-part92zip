/**
 * assetClass.ts — symbol → asset class / data-provider classification.
 *
 * React Native port of src/lib/assetClass.ts
 * ──────────────────────────────────────────
 * No modifications — this file contains only pure TypeScript business logic
 * with module-level mutable state (runtimeRouting / runtimeClasses /
 * runtimeReasons).  There are no DOM APIs, browser globals, localStorage
 * usage, or HTML-specific constructs.  All Set / RegExp / string operations
 * are fully supported by Hermes (React Native's JS engine).
 */
