/**
 * URL-keyed mock API router — React Native port of src/mock/mockApi.ts
 *
 * Given a method + pathname, returns a JSON body to serve instead of hitting
 * the real network — or `undefined` to pass the request through untouched
 * (used for anything this mock layer doesn't own, e.g. WS upgrade requests,
 * health checks, broker connection probes).
 *
 * RN compatibility changes vs the web original
 * ─────────────────────────────────────────────
 * None. All logic is pure TypeScript with no browser or DOM APIs.
 * Import paths are relative (unchanged from the web original).
 */
