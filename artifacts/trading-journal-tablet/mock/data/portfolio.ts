/**
 * Mock broker account snapshots — React Native port of src/mock/data/portfolio.ts
 *
 * RN compatibility changes vs the web original
 * ─────────────────────────────────────────────
 * • Import path updated: `@/types/broker` → `@/types/broker`
 *   The `@/` alias maps to the tablet root in tsconfig.json, so
 *   `@/types/broker` resolves to `types/broker.ts` which exists in the
 *   tablet — no logic change, just alias resolution.
 * • All data values, IDs, labels, timestamps preserved verbatim.
 */
