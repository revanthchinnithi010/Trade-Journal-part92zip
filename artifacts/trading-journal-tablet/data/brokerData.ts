/**
 * brokerData.ts — static broker trade/sync/import sample data.
 *
 * React Native port of src/data/brokerData.ts
 * ─────────────────────────────────────────────
 * No modifications. This file contains only pure TypeScript types and
 * static data arrays — no DOM APIs, browser globals, or React hooks.
 * Hermes (RN 0.81.5) fully supports all constructs used here.
 *
 * NOTE: The BrokerName type here ("Delta Exchange" | "FusionMarkets" | "Groww")
 * is distinct from the marketStore.BrokerName ("delta" | "ctrader").
 * Both are preserved exactly as on the web to avoid breaking imports.
 */
