...
import { TrendingUp, TrendingDown, Activity, ChevronRight, ChevronLeft, X } from "lucide-react";
          ...
          {!isLoading && dayTrades.map((trade) => (
            <div key={trade.id} className="flex items-center gap-3 py-3 border-b border-white/[0.06] last:border-0">
              ...
              <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${
                (trade.pnl ?? 0) >= 0 ? "bg-emerald-500/15" : "bg-red-500/15"
              }`}>
                {(trade.pnl ?? 0) >= 0
              ...
              <div className="text-right flex-shrink-0">
                <p className={`text-[14px] font-bold ${(trade.pnl ?? 0) >= 0 ? "text-emerald-400" : "text-red-400"}`}>
                  {(trade.pnl ?? 0) >= 0 ? "+" : ""}{fc(trade.pnl ?? 0)}
                </p>
